# ──────────────────────────────────────────────────────────────────────────────
#  main.py — Smart Room AI using DeepSeek
# ──────────────────────────────────────────────────────────────────────────────
...
# --------------------------------------------------------------------------- #
app = FastAPI(
    title="Smart‑Room AI (DeepSeek)",
    version="1.0.0",
    description="Smart Room automation powered by DeepSeek.",
)
...
# --------------------------------------------------------------------------- #
#   DeepSeek LLM call with retry logic
# --------------------------------------------------------------------------- #
    ...
    for attempt in range(1, max_tries + 1):
        try:
            ...
            if attempt == max_tries:
                raise HTTPException(
                    status_code=504,
                    detail="DeepSeek unavailable after retries",
                )
            log.warning(
                "DeepSeek request failed (attempt %s/%s) – retrying in %.1fs",
                attempt,
                max_tries,
                backoff,
            )
...
async def smartroom_endpoint(payload: SmartRoomInput) -> SmartRoomOutput:
    ...
    try:
        ai_text = await call_deepseek(messages)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected DeepSeek error: {exc}",
        )
