...
AI_PROVIDER = os.getenv("AI_PROVIDER", "lengua")  # default to Lengua
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY")
LENGUA_API_KEY = os.getenv("LENGUA_API_KEY")

app = FastAPI(
    title="Smart‑Room AI (OpenRouter + Lengua)",
    version="2.0.0",
    description="Smart Room automation powered by dual AI providers.",
)
...
async def call_ai(prompt: str) -> str:
    ...
    # LENGUA PROVIDER
    # -------------------------------
    if AI_PROVIDER.lower() == "lengua":
        if not LENGUA_API_KEY:
            raise HTTPException(status_code=500, detail="Missing LENGUA_API_KEY")

        headers = {
            "Authorization": f"Bearer {LENGUA_API_KEY}",
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                resp = await client.post(
                    "https://api.lengua.dev/v1/chat/completions",
                    json=payload,
                    headers=headers,
                )
            ...
            raise HTTPException(status_code=500, detail=f"AI error (Lengua): {exc}")
    ...
    # -------------------------------
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"AI error (OpenRouter): {exc}")

    else:
        raise HTTPException(status_code=500, detail="Invalid AI_PROVIDER value")
