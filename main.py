# ──────────────────────────────────────────────────────────────────────────────
#  smartroom_api.py — Copilot Version
# ──────────────────────────────────────────────────────────────────────────────
...
if not COPILOT_API_KEY:
    raise RuntimeError("COPILOT_API_KEY environment variable is required")
...
# --------------------------------------------------------------------------- #
app = FastAPI(
    title="Smart‑Room AI (Copilot)",
    version="1.0.0",
    description="Smart Room automation powered by Microsoft Copilot.",
)
...
# --------------------------------------------------------------------------- #
#   Copilot LLM call with retry logic
# --------------------------------------------------------------------------- #
async def call_copilot(messages: list[dict]) -> str:
    max_tries = 3
    backoff = 1.0
    ...
    for attempt in range(1, max_tries + 1):
        try:
            ...
            if exc.response.status_code not in {429, 502, 503, 504}:
                raise HTTPException(
                    status_code=502,
                    detail=f"Copilot API error: {exc.response.status_code}",
                )
            if attempt == max_tries:
                raise HTTPException(
                    status_code=504,
                    detail="Copilot unavailable after retries",
                )
            log.warning(
                "Copilot request failed (attempt %s/%s) – retrying in %.1fs",
                attempt,
                max_tries,
                backoff,
            )
...
async def smartroom_endpoint(payload: SmartRoomInput) -> SmartRoomOutput:
    ...
    try:
        ai_text = await call_copilot(messages)
    except HTTPException as exc:
        raise exc
    except Exception as exc:
        log.exception("Unexpected Copilot error")
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected Copilot error: {exc}",
        )
